...
1. **Persist the new key as a user env var** so scheduled runs work — verified
   NOT set at User scope as of 12/06/2026 eve (a plain `$env:HEVY_API_KEY = ...`
   only lasts for that shell; use the persistent form):
   ```powershell
